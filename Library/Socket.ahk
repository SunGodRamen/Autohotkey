/*
Socket Class
Purpose: Provides an interface for handling TCP and UDP sockets in AutoHotkey.
Author: Bentschi (2014)
Source: https://www.autohotkey.com/board/topic/94376-socket-class-%C3%BCberarbeitet/
Usage:

1. Initialize Socket: 
    sockObj := new SocketTCP() for TCP 
    
    sockObj := new SocketUDP() for UDP

2. Connect to a server: 
    sockObj.connect(host, port)

3. Bind to a specific address and port: 
    sockObj.bind(host, port)

4. Listen for incoming connections (TCP):    
    sockObj.listen([backlog])

5. Accept incoming connection (TCP):    
    newConnection := sockObj.accept()

6. Send data:
    sockObj.send(addr, length) for raw data 
    
    sockObj.sendText(message, [encoding]) for text

7. Receive data:
    lengthReceived := sockObj.recv(bufferVar, [wait])

    receivedText := sockObj.recvText([wait], [encoding])
...
For UDP:
- Enable broadcasting: 
    udpSockObj.enableBroadcast()

- Disable broadcasting: 
    udpSockObj.disableBroadcast()

Note: 
- The class uses windows socket (ws2_32) DLL calls for its functionality.
- Events like onRecv, onAccept, and onDisconnect need to be implemented 
  by the user based on their specific requirements.
- SocketTCP and SocketUDP are child classes that inherit from the main Socket class 
  with specific settings for TCP and UDP respectively.
*/
